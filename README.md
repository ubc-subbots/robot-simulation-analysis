...
1. Create a virtual environment (optional) and run `pip install -r requirements.txt` tom install requirements.
2. For an added mass matrix, we need a simpler model to analyze since the calculations are arduious. This can be done using the FeatureWorks feature in solid works, or you can model a super simplified version of the base robot like I did. Attached is an example blender file.
3. Change the top level variables to the appropriate file names and run it. This will do the calculations and print out the added mass matrix, center of buoyancy, and volume which you can copy into the `.sdf` file.
